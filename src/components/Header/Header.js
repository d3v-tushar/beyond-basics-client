import React from 'react';
import { useContext } from 'react';
import { Link, NavLink } from 'react-router-dom';
import { AuthContext } from '../../context/UserContext';
import './Header.css';

const Header = ({darkMode, setDarkMode}) => {
    const {user, logOut} = useContext(AuthContext);

    //User Log Out
    const handleLogOut = () =>{
        logOut()
        .then(() => console.log('User Logged Out'))
        .catch(error => console.error(error.message))
    };

    const handleDarkMode = (e) =>{
        if(e.target.checked){
            localStorage.setItem('dark-mode', true);
            setDarkMode(true);
        }
        else{
            localStorage.setItem('dark-mode', false);
            setDarkMode(false); 
        }
    };
    return (
        <>
            <div className="navbar bg-base-300 px-4">
                <div className="navbar-start">
                    <div className="dropdown">
                    <label tabIndex={0} className="btn btn-ghost lg:hidden">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                    </label>
                    <ul tabIndex={0} className="menu menu-compact dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52">
                    <li>
                        <NavLink to='/home'>Home</NavLink>
                    </li>
                    <li>
                        <NavLink to='/courses'>Courses</NavLink>
                    </li>
                    <li>
                        <NavLink to='/faq'>FAQ</NavLink>
                    </li>
                    <li>
                        <NavLink to='/blog'>Blog</NavLink>
                    </li>
                    </ul>
                    </div>
                    <div className='site-logo'><Link to='/' className="btn btn-ghost normal-case font-bold text-3xl"><img src="https://i.ibb.co/TMJGXH5/logo.png" className='hidden lg:grid' alt="site logo" /><p className='lg:hidden'><i><span className='text-gray-400'>B</span>B!</i></p></Link></div>
                </div>
                <div className="navbar-center hidden lg:flex">
                    <ul className="menu menu-horizontal p-0">
                    <li className='mx-2 font-semibold text-lg'>
                        <NavLink to='/home'>Home</NavLink>
                    </li>
                    <li className='mx-2 font-semibold text-lg'>
                        <NavLink to='/courses'>Courses</NavLink>
                    </li>
                    <li className='mx-2 font-semibold text-lg'>
                        <NavLink to='/faq'>FAQ</NavLink>
                    </li>
                    <li className='mx-2 font-semibold text-lg'>
                        <NavLink to='/blog'>Blog</NavLink>
                    </li>
                    </ul>
                </div>
                {/* Exp */}
                <div className="navbar-end">
                <label className="swap swap-rotate mx-2">
  
                {/* <!-- this hidden checkbox controls the state --> */}
                <input onClick={handleDarkMode} type="checkbox" />
                
                {/* <!-- sun icon --> */}
                <svg className="swap-on fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"/></svg>
                
                {/* <!-- moon icon --> */}
                <svg className="swap-off fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"/></svg>
                
                </label>
                <div className="dropdown dropdown-end">
                        <Link to='/profile'>
                        <div className="hidden lg:grid w-10 rounded-full mx-2 hover:tooltip hover:tooltip-open hover:tooltip-bottom" data-tip={user?.displayName || 'User Name'}>
                            <img className="h-10 rounded-full" src={user?.photoURL ? user.photoURL : "https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg"} alt='dp' />
                        </div>
                        </Link>
                        </div>
                        {
                        user?.uid ? <Link><button onClick={handleLogOut} className='btn btn-sm'>LogOut</button></Link> : <div className="btn-group">
                            <Link className='btn btn-sm btn-active' to='/login'>Login</Link> <Link className='btn btn-sm' to='/register'>Register</Link>
                        </div>
                        }
                </div>
            </div>
        </>
    );
};

export default Header;